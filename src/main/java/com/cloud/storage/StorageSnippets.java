...
//		byte[] data = "Hello, World!".getBytes(UTF_8);
//
//		// The name of the existing bucket to set a default KMS key for, e.g.
//		// "my-bucket"
...
//
//	/**
//	 * Example of getting information on a bucket, only if its metageneration
//	 * matches a value, otherwise a {@link StorageException} is thrown.
//	 */
...
//
//	/**
//	 * Example of getting information on a blob, only if its metageneration matches
//	 * a value, otherwise a {@link StorageException} is thrown.
//	 */
...
//
//	/**
//	 * Example of udating a blob, only if the blob's metageneration matches a value,
...
//
//	/**
//	 * Example of deleting a bucket, only if its metageneration matches a value,
//	 * otherwise a {@link StorageException} is thrown.
//	 */
...
//		boolean deleted = storage.delete(bucketName, BucketSourceOption.metagenerationMatch(bucketMetageneration));
//		if (deleted) {
...
//
//	/**
//	 * Example of deleting a blob, only if its generation matches a value, otherwise
...
//		boolean deleted = storage.delete(bucketName, blobName, BlobSourceOption.generationMatch(blobGeneration));
//		if (deleted) {
...
//		BlobId blobId = BlobId.of(bucketName, blobName);
//		BlobInfo blobInfo = BlobInfo.newBuilder(blobId).setContentType("text/plain").build();
//		ComposeRequest request = ComposeRequest.newBuilder().setTarget(blobInfo).addSource(sourceBlob1)
//				.addSource(sourceBlob2).build();
...
//		// [START copyBlob]
//		CopyRequest request = CopyRequest.newBuilder().setSource(BlobId.of(bucketName, blobName))
...
//	public Blob rotateBlobEncryptionKey(String bucketName, String blobName, String oldEncryptionKey,
//			String newEncryptionKey) {
//		// [START storageRotateEncryptionKey]
//		BlobId blobId = BlobId.of(bucketName, blobName);
//		CopyRequest request = CopyRequest.newBuilder().setSource(blobId)
//				.setSourceOptions(BlobSourceOption.decryptionKey(oldEncryptionKey))
//				.setTarget(blobId, BlobTargetOption.encryptionKey(newEncryptionKey)).build();
//		Blob blob = storage.copy(request).getResult();
...
//
//	/**
//	 * Example of reading all bytes of a blob, if generation matches a value,
//	 * otherwise a {@link StorageException} is thrown.
//	 */
...
//
//	/**
//	 * Example of creating a signed URL that is valid for 2 weeks, using the default
...
//	 * {@link SignUrlOption#signWith(ServiceAccountSigner)} option, that will be
...
//
//	/**
//	 * Example of updating information on several blobs using a single batch
//	 * request.
//	 */
//	// [TARGET update(BlobInfo...)]
//	// [VARIABLE "my_unique_bucket"]
...
//		List<Blob> updatedBlobs = storage.update(firstBlob.toBuilder().setContentType("text/plain").build(),
...
//		Acl acl = storage.updateDefaultAcl(bucketName, Acl.of(User.ofAllAuthenticatedUsers(), Role.OWNER));
...
//		Storage storage = StorageOptions.getDefaultInstance().getService();
//
//		// The name of the existing bucket to enable requester-paying for, e.g.
//		// "my-bucket"
//		// String bucketName = "my-bucket"
...
//	public void downloadFileUsingRequesterPays(String projectId, String bucketName, String srcFilename,
...
//
//		Bucket bucketWithRetentionPolicy = storage
//				.update(BucketInfo.newBuilder(bucketName).setRetentionPeriod(retentionPeriod).build());
//
//		System.out.println(
...
//		// String bucketName = "my-bucket";
//
//		Bucket bucket = storage.update(BucketInfo.newBuilder(bucketName).setDefaultEventBasedHold(false).build());
//
...
//
//		BucketInfo.IamConfiguration iamConfiguration = BucketInfo.IamConfiguration.newBuilder()
//				.setIsUniformBucketLevelAccessEnabled(false).build();
//		Bucket bucket = storage.update(BucketInfo.newBuilder(bucketName).setIamConfiguration(iamConfiguration).build());
//
